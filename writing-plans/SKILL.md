...
- 使用 `write_file(path="docs/superpowers/plans/YYYY-MM-DD-<feature-name>.md", content="...")` 保存计划
- （用户对计划位置的偏好优先于此默认值）
...
运行：`run_command(command="pytest tests/path/test.py::test_name -v")`
...
```bash
run_command(command="git add tests/path/test.py src/path/file.py")
run_command(command="git commit -m 'feat: add specific feature'")
```
````
...
**4. 写入冲突策略：** 如果有任何文件/状态/资源被多条路径写入（正常路径、fallback、定时器、干跑模式），是否已在计划中定义了写入优先级？例如：model 决策 > heuristic 决策 > 不写；或者 --no-state 时不写注入文件。在实现前定好，不要事后补。
...
**如果选择子代理驱动：**
- **必需技能：** 使用 subagent-driven-development
- 每个任务一个新子代理 + 两阶段审查

...
2. 对每个任务循环：
   - 如果任务涉及 agent、workflow、runner、adapter、ACP/MCP/CLI、bridge、transcript、stdout/stderr、output 文件、cancel/timeout 或进程生命周期，先在主代理中使用 `agent-adapter-contracts` 完成契约审计，并把契约矩阵、必须保留的旧语义、真实路径验证要求和异常路径要求写入实现子代理的任务文本
...
- **flow-review** - 审查子代理与 requesting mode 的代码审查模板
...
当存在 2 个以上互不关联的问题、测试失败、bug 或独立任务，需要并行分派子代理并发排查或执行时，使用 parallel dispatch mode。
...
Reasonix 的 `spawn_subagent` 和 `run_skill` 标记为 `parallelSafe: true` 时，可在同一 tool batch 中同时发出多个调用，它们会并发执行。
...
2. **构造自包含的任务描述**
   每个子代理没有你的会话上下文，task 必须包含范围、目标、必要背景、约束和输出格式。

3. **并行分派**
   在一次模型回合中同时发出所有子代理调用，不要等一个完成再发下一个。

4. **收集结果**
   阅读每个子代理输出，确认成功、发现和修改建议。

...
涉及 runner、adapter、controller、agent workflow、ACP/MCP/CLI、transcript、stdout/stderr、output 文件、cancel/timeout 时，测试还必须保护协议契约：

- mock 只能替代外部 agent 或外部命令，不能 mock 掉本项目正在修改的 runner/adapter 本身。
- fake ACP/MCP 必须实现关键协议语义，例如 JSON-RPC 握手、session 创建、prompt、update 或 close。
- fake Codex/CLI 必须覆盖 stdout 与指定 output 文件分离的语义，尤其是 `-o` 这类产物路径。
- 至少覆盖一个相关失败路径：startup failure、early exit、non-zero exit、closed stdin/EPIPE、cancel 或 timeout。
- 测试名称应说明被保护的契约，而不是只说明实现细节。
...
- [ ] 覆盖了边界情况和错误场景
- [ ] 如果涉及 agent/adapter/protocol，测试覆盖了真实 runner 语义、output/transcript 契约和相关失败路径
...
- 在生产类中添加仅测试用的方法
- 在不理解依赖的情况下使用 mock
- 用 shell echo fake 代替协议 runner 验收
- 绕过正在修改的 runner，只测试 controller 或 helper 的孤立行为

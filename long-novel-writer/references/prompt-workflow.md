...
**在阶段 A 之前执行**（如果 `metadata.md` 的 `润色模式` 不为 `关闭`）：

章节正文生成后，调用 `article-polisher` 技能进行去 AI 化润色。润色是可选步骤（严肃文学模式除外，强制执行），但**必须在阶段 A 之前完成**——润色后的正文是 L1 brief、canon 提取、实体更新的唯一事实来源。

润色约束详见 `SKILL.md` 第 4 步，核心输入材料：
- 本章正文
- `naming.md` 规范名称表（最高优先级硬约束）
- 出场角色 `dialogue-samples/<slug>.md`（对话风格保持）
- `metadata.md` 的体裁模式/类型/基调（文体适配）
- 出场实体档案的「硬约束」段（canon 保护）
- 本章伏笔条目 `foreshadowing_set` / `foreshadowing_paid`（伏笔保护）
- 润色字数容差（缺省 ±5%）

润色完成后执行快速校验：
1. grep 检查禁用变体
2. 字数偏差检查（`tr -d '[:space:]' | wc -m`）
3. 抽查关键伏笔句段
...
**章节正文生成（及润色）后不要直接落盘**，按以下三阶段执行（对齐 `memory-protocol.md` §5）：

...
- 单卷章数：
- 体裁模式：网文 / 严肃文学 / 通用
- 润色模式：开启              # 开启/关闭；章节生成后是否调用 article-polisher 去 AI 化润色。关闭仅限用户明确要求
- 润色字数容差：0.05          # 润色后字数浮动比例，缺省 5%。润色不改情节，字数变化通常小
- 禁用风格/桥段：
```
...
- `润色模式`：控制章节生成后是否调用 `article-polisher` 技能进行去 AI 化润色。可选值：`开启` / `关闭`。缺省为 `开启`。关闭仅限用户明确要求不做润色。
- `润色字数容差`：润色后字数浮动比例，缺省 0.05（±5%）。润色不改情节和事件，字数变化通常远小于写作容差。
- 体裁模式联动：`严肃文学` 模式下润色强制执行（`润色模式=关闭` 时也忽略），因为文学性对文本质量要求更高。
